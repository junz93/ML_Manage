...
//	                System.out.println("----------收到消息内容是:::"+content);
	                buf.limit(oldLimit2);     
//	                System.out.println(content); //logger record it
	                
	                int i1 = content.indexOf(";");
	                String length1 = content.substring(0, i1);
	                int headLength = Integer.valueOf(length1);
	                
	               
	                content = content.substring(i1+1);
	                String head = content.substring(0, headLength);
	                String doc = content.substring(headLength);
	                
	                if(content.contains("className`")){
	                	
	                	int index1 = content.indexOf("`");
	                	int index2 = content.indexOf("~");
	                	String className = head.substring(index1+1, index2);
	                	head = head.substring(index2+1);
	                	
	                	WsnMsg msg = (WsnMsg) Class.forName(className).newInstance(); //得到实例
		                if(content!=null && content != "" && content.length() != 0){
		                	msg.stringToMsg(head, doc);  //为各个域赋值
		                }		                	
		                out.write(msg);       //写入通道
